...
TEST_CASE("Unit_hipMemPoolExportToShareableHandle_multiproc") {
  ...
  REQUIRE(sharedMemoryCreate(shmName, sizeof(mempoolIpcShmStruct), &shmInfo) == 0);
  auto *shmData = reinterpret_cast<mempoolIpcShmStruct *>(shmInfo.addr);
  ...
  const char *spawnArgs[] = {
      exePath.c_str(),
      "Unit_hipMemPoolExportToShareableHandle_multiproc_child",
      nullptr};
  REQUIRE(spawnProcess(&childProc, exePath.c_str(),
                       const_cast<char *const *>(spawnArgs)) == 0);
  ...
  checkSysCallErrors(sockObj.sendShareableHdl(sharedHandle, childProc));

  int exitCode = waitProcess(&childProc);
  REQUIRE(exitCode == 0);
...
TEST_CASE("Unit_hipMemPoolExportToShareableHandle_multiproc_child",
  ...
  sharedMemoryInfo shmInfo{};
  if (sharedMemoryOpen(shmName, sizeof(mempoolIpcShmStruct), &shmInfo) != 0) {
