...
#undef CL_FILTER_NONE
  ...
  switch (pResDesc->resType) {
    ...
    case hipResourceTypeLinear: {
      ...
      // verify the pointer is a known HIP allocation because on APUs, host pointers may pass the
      // earlier checks but are not valid device allocations
      if (buffer == nullptr) {
    ...
    case hipResourceTypePitch2D: {
      ...
      if (buffer == nullptr) {
        amd::Device* curDev = hip::getCurrentDevice()->devices()[0];
        if (amd::MemObjMap::FindMemObj(pResDesc->res.pitch2D.devPtr, nullptr, curDev) == nullptr) {
          return hipErrorInvalidValue;
        }
      }
