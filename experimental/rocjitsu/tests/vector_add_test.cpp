...
TEST(VectorAddStressTest, AllCUsGoldenReference) {
  ...
  vm->memory()->load_image(reinterpret_cast<const uint8_t *>(text->data()), text->size(),
                           CODE_ADDR);
  ...
  vm->memory()->load_image(reinterpret_cast<const uint8_t *>(A.data()), vec_bytes, A_ADDR);
  vm->memory()->load_image(reinterpret_cast<const uint8_t *>(B.data()), vec_bytes, B_ADDR);
  std::vector<float> zeros(N, 0.0f);
  vm->memory()->load_image(reinterpret_cast<const uint8_t *>(zeros.data()), vec_bytes, C_ADDR);
  ...
  vm->memory()->load_image(reinterpret_cast<const uint8_t *>(&args), sizeof(args), KERNARG_ADDR);
  ...
  for (uint32_t xi = 0; xi < TOTAL_XCDS; ++xi) {
    ...
    vm->soc()->xcd(xi)->command_processor()->enqueue(pkt);
  }
  ...
  for (uint32_t i = 0; i < N; ++i) {
    float actual = std::bit_cast<float>(vm->memory()->read32(C_ADDR + i * sizeof(float)));
