...
struct KernelExecFixture {
  ...
  void setup(const char *kernel_name) {
  ...
  amdgpu::CommandProcessor *cp(uint32_t xcd = 0) {
    return vm->soc()->xcd(xcd)->command_processor();
  }
  ...
  void dispatch_all_xcds(uint32_t total_wgs) {
    uint32_t num_xcds = vm->soc()->num_xcds();
...
void run_matmul_stress(const char *kernel_name, unsigned N) {
...
TEST(MatmulStressTest, Cdna4TopologyDispatchAndHalt) {
  ...
  vm->memory()->write32(CODE_ADDR, SOPP_S_ENDPGM);
