...
struct VmFixture {
  ...
  VmFixture(const std::string &arch = "cdna3", uint32_t num_cus = 1, uint32_t num_wf_slots = 10) {
    std::string json = R"({"max_ticks":10000,"num_threads":1,"vm":{)"
                       R"("arch":")" +
                       arch +
                       R"(","gpu":{"num_xcds":1,"xcd":{"num_shader_engines":1,)"
                       R"("shader_engine":{"num_compute_units":)" +
                       std::to_string(num_cus) + R"(,"compute_unit":{"num_wf_slots":)" +
                       std::to_string(num_wf_slots) +
                       R"(,"sgprs_per_wf":104,"vgprs_per_wf":256,"lds_size_kb":64}}}}}})";
  ...
  amdgpu::Xcd *xcd(uint32_t idx = 0) { return vm->soc()->xcd(idx); }
  amdgpu::ShaderEngine *se(uint32_t idx = 0) { return vm->soc()->xcd(0)->shader_engine(idx); }
  amdgpu::GpuMemory *mem() { return vm->memory(); }
...
TEST(VmLifecycleTest, CreateAndDestroy) {
  std::string json = R"({"max_ticks":10000,"num_threads":1,
    "vm":{"arch":"cdna3","gpu":{"num_xcds":1,"xcd":{"num_shader_engines":2,"shader_engine":{"num_compute_units":3,
...
TEST_P(IsaTest, RunToCompletion) {
  std::string json = R"({"max_ticks":10000,"num_threads":1,"vm":{
    "arch":")" + arch() +
                     R"(","gpu":{"num_xcds":1,"xcd":{"num_shader_engines":1,
    "shader_engine":{"num_compute_units":1,
    "compute_unit":{"num_wf_slots":10,"sgprs_per_wf":104,"vgprs_per_wf":256,"lds_size_kb":64}}}}}})";
