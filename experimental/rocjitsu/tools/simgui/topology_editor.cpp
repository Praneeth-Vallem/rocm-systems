...
void TopologyEditor::load_from_simulation(VirtualMachine *vm) {
  if (!vm)
    return;
...
void TopologyEditor::render() {
  ...
  for (const auto &node : nodes_) {
    ...
    if (node.input_attr_id >= 0) {
      ...
      const char *label = (node.type == ComponentType::COMPUTE_UNIT) ? "dispatch_in" : "mem_in";
      ImGui::Text("%s", label);
    ...
    if (node.output_attr_id >= 0) {
      ...
      const char *label =
          (node.type == ComponentType::COMMAND_PROCESSOR) ? "dispatch_out" : "mem_out";
      ImGui::Text("%s", label);
