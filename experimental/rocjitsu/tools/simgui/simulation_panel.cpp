...
void SimulationPanel::render_wf_state(VirtualMachine *vm) {
  if (!vm)
    return;
...
void SimulationPanel::render_timeline(VirtualMachine *, uint64_t) {
  if (history_.empty()) {
...
void SimulationPanel::record_tick(VirtualMachine *vm, uint64_t tick) {
  if (!vm)
    return;
