...
/// This prevents nonsensical connections (e.g., wiring a memory response
/// port to a dispatch port). UNTYPED is the default and allows any
/// connection (backward-compatible with existing code).
enum class PortProtocol : uint8_t {
  UNTYPED,        ///< No protocol constraint (default, backward-compatible).
  MEMORY_REQ,     ///< Memory request (load/store/atomic).
  MEMORY_RESP,    ///< Memory response (data return, completion).
  DISPATCH,       ///< Workgroup/wavefront dispatch command.
  COMPUTE_STATUS, ///< Compute unit status report (idle, busy, done).
};
...
public:
  ...
  /// Concrete components (e.g., VirtualMachine, CommandProcessor) override
...
public:
  ...
  /// Subclasses (e.g., VirtualMachine) override for custom lifecycle.
  void run() override;
