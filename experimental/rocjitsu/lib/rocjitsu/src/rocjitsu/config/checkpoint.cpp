...
/// @brief Serialize the VM configuration into a FlatBuffer SimulationConfig.
flatbuffers::Offset<fb::SimulationConfig>
serialize_config(flatbuffers::FlatBufferBuilder &builder, const VirtualMachine &vm,
                 const simdojo::SimulationEngine::Config &engine_config) {
  const auto &cfg = vm.config();
  const auto &soc = cfg.soc;
  const auto &se_cfg = soc.xcd.shader_engine;
  const auto &cu_cfg = se_cfg.compute_unit;

  auto arch_str = builder.CreateString(arch_to_string(soc.arch));

  auto fb_cu = fb::CreateComputeUnitConfig(builder, cu_cfg.num_wf_slots, cu_cfg.sgprs_per_wf,
                                           cu_cfg.vgprs_per_wf, cu_cfg.lds_size_kb);
  auto fb_se = fb::CreateShaderEngineConfig(builder, se_cfg.num_compute_units, fb_cu);
  auto fb_xcd = fb::CreateXcdConfig(builder, soc.xcd.num_shader_engines, fb_se);
  auto fb_gpu = fb::CreateAmdgpuConfig(builder, soc.num_xcds, soc.num_iods, fb_xcd);
...
void save_checkpoint(const std::string &path, const VirtualMachine &vm, uint64_t tick,
                     const simdojo::SimulationEngine::Config &engine_config) {
  ...
  vm.memory()->for_each_page([&](uint64_t addr, const auto &page) {
  ...
  auto config_offset = serialize_config(builder, vm, engine_config);
...
LoadedConfig restore_checkpoint(const std::string &path) {
  ...
  // Rebuild VM and engine config from the stored configuration.
  ...
  engine_config.verbose = fb_config->verbose();
    ...
    if (auto *pages = mem_state->pages()) {
      for (auto *page : *pages) {
        if (page->data() && page->data()->size() > 0) {
          vm_ptr->memory()->load_image(page->data()->data(), page->data()->size(), page->address());
        }
      }
    }
  }
