...
struct rj_vm_t : RefCounted {
  ...
  VirtualMachine *vm = nullptr; ///< Cached pointer into topology root.
};

namespace {

/// @brief Create an rj_vm_t from a loaded VM and engine config.
rj_status_t create_vm(simdojo::SimulationEngine::Config engine_config,
                      std::unique_ptr<VirtualMachine> vm, rj_vm_t **handle) {
  if (!vm)
  ...
  s->vm = vm.get();
  s->engine = std::make_unique<simdojo::SimulationEngine>(engine_config);
  s->engine->topology().set_root(std::move(vm));
  s->engine->build();
...
rj_status_t rj_vm_create(const char *json_path, const char *schema_path, rj_vm_t **vm) {
  ...
  try {
    ...
    return create_vm(loaded.engine_config, std::move(loaded.vm), vm);
...
rj_status_t rj_vm_save_checkpoint(const rj_vm_t *vm, const char *path, uint64_t tick) {
  ...
  if (!vm->vm)
    return ROCJITSU_STATUS_ERROR;
  try {
    config::save_checkpoint(path, *vm->vm, tick, vm->engine_config);
    return ROCJITSU_STATUS_SUCCESS;
  } catch (const std::exception &) {
    return ROCJITSU_STATUS_ERROR;
  }
