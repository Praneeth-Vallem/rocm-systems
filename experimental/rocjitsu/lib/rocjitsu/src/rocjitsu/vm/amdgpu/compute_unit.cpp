...
ComputeUnitCore::ComputeUnitCore(std::string name, const Config &config, GpuMemory *memory,
                                 L2CacheController *l2, uint32_t wf_size)
    : simdojo::CompositeComponent(std::move(name)), config_(config), memory_(memory),
      ...
      global_mem_pipeline_(&l1_vector_), local_mem_pipeline_(&lds_) {
  if (!decoder_)
  ...
  // Dispatch IN port: CP sends activation messages here.
  dispatch_in_ = add_port(std::make_unique<simdojo::Port>(
      "dispatch_in", 0, this, simdojo::PortDirection::IN, simdojo::PortProtocol::DISPATCH));
  dispatch_in_->set_handler([this](simdojo::Tick, simdojo::Message *) { activate(); });

  // L2 request OUT port: structural connection to shared L2 cache.
  l2_req_ = add_port(std::make_unique<simdojo::Port>("l2_req", 1, this, simdojo::PortDirection::OUT,
                                                     simdojo::PortProtocol::MEMORY_REQ));
}

std::unique_ptr<ComputeUnitCore> ComputeUnitCore::create(std::string name, const Config &config,
                                                         GpuMemory *memory, L2CacheController *l2,
