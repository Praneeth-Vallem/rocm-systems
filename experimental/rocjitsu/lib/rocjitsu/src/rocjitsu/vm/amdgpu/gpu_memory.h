...
public:
  ...
  explicit GpuMemory(std::string name) : simdojo::SparseMemory(std::move(name)) {
    hbm_in_ = add_port(std::make_unique<simdojo::Port>(
        "hbm_in", 0, this, simdojo::PortDirection::IN, simdojo::PortProtocol::MEMORY_REQ));
  }

  /// @brief Return the HBM input port (for topology wiring).
  /// @returns Pointer to the HBM input port.
  simdojo::Port *hbm_in_port() { return hbm_in_; }

private:
  simdojo::Port *hbm_in_ = nullptr;
};
