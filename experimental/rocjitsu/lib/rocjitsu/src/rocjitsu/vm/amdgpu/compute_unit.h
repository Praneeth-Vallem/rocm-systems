...
public:
  ...
  /// @param l2 Shared L2 cache controller (owned by the XCD's L2Cache component).
  ...
  create(std::string name, const Config &config, GpuMemory *memory, L2CacheController *l2,
  ...
  ///
  /// @details In FUNCTIONAL mode, drains all active wavefronts via step()
  /// until none remain, then retires halted wavefronts and notifies the
  /// idle callback. In CLOCKED mode (future), processes one pipeline cycle.
  ...
  /// @brief Return the dispatch IN port (for topology wiring from CP).
  /// @returns Pointer to the dispatch input port.
  simdojo::Port *dispatch_in_port() { return dispatch_in_; }

  /// @brief Return the L2 request OUT port (for topology wiring to L2).
  /// @returns Pointer to the L2 request output port.
  simdojo::Port *l2_req_port() { return l2_req_; }

protected:
  ComputeUnitCore(std::string name, const Config &config, GpuMemory *memory, L2CacheController *l2,
                  uint32_t wf_size);
  ...
  simdojo::Port *dispatch_in_ = nullptr; ///< Dispatch activation from CP.
  simdojo::Port *l2_req_ = nullptr;      ///< L2 cache request (structural).
};
...
///
/// In FUNCTIONAL mode, advance() exhausts all active wavefronts in a single
/// call (zero per-instruction event overhead). activate() schedules an engine
/// event that calls advance(). When advance() returns false (no more work),
/// the idle callback is notified so the command processor can detect
/// completion.
///
...
public:
  ...
  /// @brief Execute work until no active wavefronts remain.
  ///
  /// @details In FUNCTIONAL mode, drains all wavefronts via step(), retires
  /// halted wavefronts, and returns whether more work remains. If the CU is
  /// idle after draining, fires the on_idle callback.
  /// @retval true More work remains (e.g., new dispatch arrived during execution).
  ...
  void activate() override {
    this->engine()->schedule_event(&work_event_, this->engine()->global_time() + 1);
  }
...
private:
  simdojo::Event work_event_{this, simdojo::EventType::TIMER_CALLBACK,
                             [this](simdojo::Tick now, simdojo::Message *) {
                               if (advance())
                                 this->engine()->schedule_event(&work_event_, now + 1);
                             }};
};
