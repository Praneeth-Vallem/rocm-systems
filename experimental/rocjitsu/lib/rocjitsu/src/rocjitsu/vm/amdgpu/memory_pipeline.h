...
/// (SMEM), GlobalMemPipeline (FLAT/MUBUF/MTBUF), and LocalMemPipeline (DS).
/// Each subclass knows which DynamicInstState subtype to cast to and which
...
public:
  ...
  ///
  /// Processes returned instructions (complete_access + counter decrement),
  /// then initiates new requests from the issued queue.
  void tick() {
    if (!returned_.empty()) {
...
public:
  ...
  /// @param l2 L2 Cache Controller (for store bypass), not owned.
  ScalarMemPipeline(L1ScalarCache *l1, L2CacheController *l2)
...
///
/// Routes directly to the L1 Vector Cache (V$).
class GlobalMemPipeline : public MemoryPipeline {
public:
  /// @param l1 L1 Vector Cache (V$), not owned.
  explicit GlobalMemPipeline(L1VectorCache *l1) : MemoryPipeline(WaitCounterType::VMCNT), l1_(l1) {}
